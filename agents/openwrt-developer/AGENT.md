...
# Domain Rules
1. **POSIX Strictness**: Use `#!/bin/sh` (ash). NO bashisms (no arrays, no `[[ ]]`). Always quote variables.
...
4. **Flash Wear**: Write runtime state/volatile data to `/tmp` or `/var/run` (RAM). NEVER write frequent states to flash.
